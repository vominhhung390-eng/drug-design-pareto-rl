...
- EGFR/VEGFR2：`models/oracles/` 提供论文使用的固定历史预测器并由一键预检核验哈希；恢复的BindingDB API快照不是精确历史训练行，只用于显式选择的替代重训条件。
- 分子对接：四个正式PDBQT受体、原始PDB/晶体配体、box配置、协议验证和Vina调用脚本。历史Vina 1.1.2可执行文件属于用户自行取得的第三方运行时，不在公共仓库中再分发。

除缺少原始训练行、因而必须保留的两个历史EGFR/VEGFR2预测器外，所有派生缓存、模型权重和完整正式结果均由 `scripts/reproduce_all.ps1` 重建；`reference_results/` 仅作为输出核对参考，不是训练输入。
