...
if os.name == "nt":
    ...
    def _popen_init_without_window(self, *args, **kwargs):
        kwargs["creationflags"] = (
            kwargs.get("creationflags", 0) | subprocess.CREATE_NO_WINDOW
        )
        _popen_init(self, *args, **kwargs)

    subprocess.Popen.__init__ = _popen_init_without_window
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Parallel workers for independent multi-start L-BFGS runs"
)
parser.add_argument(
    "--no_early_stopping",
    dest="early_stopping",
...
def robust_multi_restart_optimizer(
    ...
    if len(top_point_idxs) == 0:
        top_point_idxs = np.arange(len(y_train))
    chosen_top_point_indices = np.random.choice(
        top_point_idxs,
        size=num_good_starts,
        replace=len(top_point_idxs) < num_good_starts,
    )
    ...
    jobs = [(i, x, src) for i, (x, src) in enumerate(zip(all_points, point_sources))]
    if workers > 1:
        with ThreadPoolExecutor(max_workers=workers) as executor:
            completed = executor.map(optimize_one, jobs)
            completed = list(completed)
    else:
        completed = [optimize_one(job) for job in jobs]

    opt_results = []
    for i, src, res in completed:
        opt_results.append(res)

        fun_value = float(np.asarray(res.fun).sum())
        message = res.message.decode() if isinstance(res.message, bytes) else str(res.message)

        if logger is not None:
            logger.info(
                f"Iter#{i} t={time.time()-start_time:.1f}s: val={fun_value:.2e}, "
                f"init={src}, success={res.success}, msg={message}")

        # Potentially do early stopping
        # Good results succeed, and stop due to convergences, not low gradients
        result_is_good = res.success and ("REL_REDUCTION_OF_F_<=_FACTR*EPSMCH" in message)
        if result_is_good:
            num_good_results += 1
    ...
    sorted_results = sorted(successful_results, key=lambda r: float(np.asarray(r.fun).sum()))
    x_out = [res.x for res in sorted_results[:num_pts_to_return]]
    opt_vals_out = [float(np.asarray(res.fun).sum())
                    for res in sorted_results[:num_pts_to_return]]
    return np.array(x_out), opt_vals_out


def gp_opt(gp_file, data_file, save_file, n_out, logfile, n_starts=20, early_stopping=True, workers=1):
