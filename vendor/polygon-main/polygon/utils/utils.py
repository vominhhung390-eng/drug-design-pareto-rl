...
# IO STUFF
################################################################################
def _infer_vae_model_params(state_dict):
    """Infer VAE architecture parameters from a saved state dict."""
    if "encoder_rnn.weight_ih_l0" not in state_dict:
        return None

    q_weight = state_dict["encoder_rnn.weight_ih_l0"]
    d_weight = state_dict["decoder_rnn.weight_hh_l0"]
    q_bidir = "encoder_rnn.weight_ih_l0_reverse" in state_dict

    q_layers = [
        key for key in state_dict
        if key.startswith("encoder_rnn.weight_ih_l") and not key.endswith("_reverse")
    ]
    d_layers = [
        key for key in state_dict
    ...
    return {
        "q_bidir": q_bidir,
        "q_d_h": q_weight.shape[0] // 3,
        "q_n_layers": len(q_layers),
        "d_d_h": d_weight.shape[0] // 3,
        "d_n_layers": len(d_layers),
        "d_z": state_dict["q_mu.weight"].shape[0],
    }
...
def load_model(model_class, model_definition, device, model_params=None, copy_to_cpu=True):
    """
        ...
        """
    if "cpu" in device:
        state_dict = torch.load(model_definition, map_location="cpu")
    else:
        state_dict = torch.load(model_definition)

    if model_params is None and getattr(model_class, "__name__", "") == "VAE":
        model_params = _infer_vae_model_params(state_dict)
