#!/usr/bin/env python
"""Summarize the pre-registered V4-B versus official POLYGON confirmation."""
...
RUN_ROOT = ROOT / "results" / "own_method_v4" / "confirmation_10240"
CONFIG = json.loads(
    (ROOT / "config" / "v4_raw_mean_confirmation_10240.json").read_text(
        encoding="utf-8"
    )
)
SEEDS = CONFIG["seeds"]
...
def load_own(seed: int) -> dict:
    run = RUN_ROOT / f"v4_b_raw_mean_seed{seed}" / "evaluation"
...
def main() -> None:
    ...
    quality_ci = stats.t.interval(
        0.95,
        len(quality_diff) - 1,
        loc=quality_diff.mean(),
        scale=stats.sem(quality_diff),
    )
    ttest = stats.ttest_rel(own.hv, polygon.hv)
    quality_ttest = stats.ttest_rel(own.quality_hv, polygon.quality_hv)
    ...
    paired = pd.DataFrame(
        [
            {
                "comparison": "own_v4_b_raw_mean_minus_polygon",
                ...
                "quality_hv_ci95_low": quality_ci[0],
                "quality_hv_ci95_high": quality_ci[1],
                "quality_hv_paired_t_p": quality_ttest.pvalue,
    ...
    lines = [
        "# V4-B prospective paired confirmation (10,240 oracle calls)",
        "",
        "Seeds 82-91 and all acceptance gates were recorded before these runs. No tuning used these results.",
        "",
        "| method | HV mean +/- SD | validity | quality HV mean +/- SD | quality pass rate |",
        "|---|---:|---:|---:|---:|",
        f"| Own V4-B raw-mean | {mean_sd(own.hv)} | {own.validity.mean():.4f} | {mean_sd(own.quality_hv)} | {own.quality_pass_rate.mean():.4f} |",
        ...
        f"- Wins: {(hv_diff > 0).sum()}/10; paired t-test p={ttest.pvalue:.6f}; Wilcoxon p={wilcoxon.pvalue:.6f}.",
        f"- Quality-HV difference: {quality_diff.mean():+.4f}, 95% CI [{quality_ci[0]:+.4f}, {quality_ci[1]:+.4f}], paired p={quality_ttest.pvalue:.6f}.",
        f"- Acceptance gates: {'PASS' if all(gates.values()) else 'FAIL'}.",
        "- Decision: promote V4-B raw-mean as the selected formal own method.",
        "- Caveat: quality pass rate is lower than POLYGON, although quality-constrained HV is significantly higher.",
    ]
